...
Color getColorAt(Vect intersection_position, Vect intersecting_ray_direction, vector<Object*> scene_objects, int index_of_winning_object, vector<Source*> light_sources, double accuracy, double ambientlight) {
	...
	return final_color.clip();//.gammaCorrection()
}
...
int main (int argc, char *argv[]) {
	...
	Color turquoise (0.25,0.87,0.81,0.3);
	...
	Vect posB2(-2,0,5);
	Sphere boule2 (posB2,0.5,turquoise);
	...
	//scene_objects.push_back(dynamic_cast<Object*>(&boule2));
	...
	for (int x = 0; x < width; x++) {
		for (int y = 0; y < height; y++) {
				...
				for (int aay = 0; aay < aadepth; aay++) {
					...
					if (aadepth == 1) {
						...
						if (width > height) {
							...
							// the image is square
							xamnt = (x + (double)aax/((double)aadepth - 1)-0.5)/width;
							yamnt = ((height - y) + (double)aax/((double)aadepth - 1)-0.5)/height;
						}
					}
			...
			int thatone = ((height - y - 1) * width + x);

			image[thatone * 3 + 0] = std::min(255., avgRed*255);
            image[thatone * 3 + 1] = std::min(255., avgGreen*255);
            image[thatone * 3 + 2] = std::min(255., avgBlue*255);
		}
	}
	
	//savebmp("scene_anti-aliased.bmp",width,height,dpi,pixels);
	stbi_write_png("image_test.png", width, height, 3, &image[0], 0);
